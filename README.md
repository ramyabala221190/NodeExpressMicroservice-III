...
To help with this conversion, we have written a mapper.ts to convert the UserDocument(which contains all mongoose details) into
UserModel, which is a plain JS object.
...
`    const userObj=user.toObject();`
Next, we have used that object: userObj to convert all ObjectId fields into string fields using the `toString()`.
...
`"prelocal": "swagger-cli bundle node_modules/@ramyabala221190/api-contracts/dist/openapi/user/openapi.json -o bundled-user.json -t json"`
